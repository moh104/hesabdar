...
std::shared_ptr<Currency> operator+(std::shared_ptr<Currency> currency1, std::shared_ptr<Currency> currency2)
{
    ...
    double result = currency1->get_amount() + currency2->get_amount();
...
std::ostream& operator<<(std::ostream& out, const std::shared_ptr<Currency>& currency) {
    out << currency->get_amount() << " " << currency->get_unit();
    return out;
}
